 ...
 */
export function validateTweetLink(url) {
  if (!url || !url.trim()) {
 ...
 * Full validation happens server-side via parseCronExpression in GAS.
 * @param {string} cron
 * @returns {string|null}  null if valid; error message string if invalid
 */
 ...
 */
export function updateCharCount(textarea, counter, limit = 280) {
